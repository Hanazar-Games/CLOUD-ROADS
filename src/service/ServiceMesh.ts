import { BoxGeometry, BufferGeometry, Color, Float32BufferAttribute, InstancedMesh, Matrix4, Mesh, MeshStandardMaterial, type Scene } from 'three';
...
export class ServiceMesh {
  ...
  constructor(scene: Scene, private readonly options: Readonly<WorldOptions> = DEFAULT_OPTIONS) {
    for (const mesh of [this.pavement, this.buildings, this.windows, this.markings, this.lights]) {
      mesh.visible = false; mesh.receiveShadow = true; mesh.castShadow = mesh === this.buildings;
      scene.add(mesh);
    }
    for (const mesh of [this.buildings, this.windows, this.markings, this.lights]) mesh.count = 0;
  }
  ...
  update(sites: readonly ServiceArea[], version: number, originX: number, originZ: number): void {
    if (this.version !== version) {
      ...
      for (const mesh of [this.buildings, this.windows, this.markings, this.lights]) {
  ...
  private build(pad: ServicePad): void {
    ...
    box(15, 28, 5.5, 24, 0.6, 32, 0x334b48);
  ...
  dispose(): void {
    for (const mesh of [this.pavement, this.buildings, this.windows, this.markings, this.lights]) { mesh.removeFromParent(); mesh.geometry.dispose(); mesh.material.dispose(); }
    for (const mesh of [this.buildings, this.windows, this.markings, this.lights]) mesh.dispose();
  }
}
