...
export interface TrailerConfig { length: number; wheelbase: number; hitchAlong: number; front: number; wheels: readonly WheelPoint[] }
export interface VehicleProfile {
  name: string; shape: 'roadster' | 'sedan' | 'suv' | 'truck' | 'tractor' | 'bus' | 'motorcycle';
...
const axles = (track: number, positions: number[]): WheelPoint[] => positions.flatMap((along, axle) =>
  [-1, 1].map(side => ({ x: side * track / 2, along, steer: axle === 0 })));
...
const trailer = (length: number): TrailerConfig => {
  ...
  return { length: length - 3.6, wheelbase, hitchAlong: -1.8, front: 1,
