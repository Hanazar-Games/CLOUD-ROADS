...
test('switches all four seasons in place without generating new chunks or resetting the car', async ({ page }) => {
  ...
  for (const [season, name] of [['spring', '春季'], ['autumn', '秋季'], ['winter', '冬季'], ['summer', '夏季']]) {
    await (await control(page, page.locator('#season-kind'))).selectOption(season);
  ...
  await (await control(page, page.locator('#season-kind'))).scrollIntoViewIfNeeded();
...
test('renders snow, freezes its motion, and shelters precipitation inside tunnels', async ({ page }) => {
  ...
  const clip = { x: 700, y: 200, width: 500, height: 300 }, snow = await sceneShot(page, { clip });
  await page.waitForTimeout(400); expect((await sceneShot(page, { clip })).equals(snow)).toBe(false);
  await (await control(page, page.locator('#pause'))).click(); const frozen = await sceneShot(page, { clip });
  await page.waitForTimeout(400); expect((await sceneShot(page, { clip })).equals(frozen)).toBe(true);
  await (await control(page, page.locator('#pause'))).click(); await (await control(page, page.locator('#tunnel-view'))).click();
  await (await control(page, page.locator('#speed'))).fill('20'); await (await control(page, page.locator('#world'))).focus(); await page.keyboard.down('KeyW');
  ...
  await (await control(page, page.locator('#home'))).click(); await expect(metric('Snow visible')).toHaveText('yes');
  expect(errors).toEqual([]);
});
