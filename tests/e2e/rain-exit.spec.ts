...
test('wipes accumulated windshield water and freezes it while paused', async ({ page }) => {
  ...
  await expect(page.locator('#drive-toggle')).toBeEnabled(); await page.locator('#drive-toggle').click();
  ...
  await page.locator('#controls-toggle').click(); await page.locator('#vehicle-wipers').selectOption('high');
  await page.locator('#controls-toggle').click(); await page.locator('#world').focus();
  ...
  await page.locator('#controls-toggle').click(); await page.locator('#vehicle-kind').selectOption('motorcycle');
...
test('leaves a parked vehicle on a high bridge with F and boards it without resetting the trip', async ({ page }) => {
  const errors: string[] = []; page.on('pageerror', e => errors.push(e.message));
  const metric = (name: string) => page.locator(`[data-metric="${name}"]`);
  await page.goto('/?seed=VALLEY-22'); await page.locator('#terrain-kind').selectOption('forest');
  await page.getByRole('button', { name: '应用并返回起点' }).click();
  await expect(page.locator('#bridge-view')).toBeEnabled(); await page.locator('#bridge-view').click();
  await page.locator('#drive-toggle').click(); await expect(metric('Travel mode')).toHaveText('driving');
  ...
  await page.locator('#release-open').click(); await page.keyboard.press('KeyF');
  await expect(metric('Travel mode')).toHaveText('driving'); await page.keyboard.press('Escape'); await page.locator('#world').focus();
  await page.keyboard.press('KeyF'); await expect(metric('Travel mode')).toHaveText('walking');
  ...
  await page.locator('#controls-toggle').click(); await page.locator('#random-world').click();
...
test('keeps lightweight antialiasing and cloud detail through resizing and context restoration', async ({ page }) => {
  ...
  await page.locator('#antialiasing').selectOption('1'); await page.locator('#cloud-quality').selectOption('12');
  ...
  await page.locator('#antialiasing').selectOption('0'); await expect(metric('FXAA')).toHaveText('off');
