...
test('unlocks audio explicitly, pauses in announcements, and mutes without creating another context', async ({ page }) => {
  ...
  await (await control(page, page.locator('[data-settings-target="audio"]'))).click();
  ...
  await (await control(page, page.locator('#sfx-volume'))).fill('0'); await (await control(page, page.locator('#music-volume'))).fill('100');
  await expect.poll(level).toBeGreaterThan(0.005);
  expect(await level()).toBeLessThan(0.5);
  await (await control(page, page.locator('#sfx-volume'))).fill('100'); await (await control(page, page.locator('#music-volume'))).fill('0');
  ...
  await expect.poll(state).toEqual(['running']);
  await (await control(page, page.locator('#sfx-volume'))).fill('0'); await (await control(page, page.locator('#music-volume'))).fill('0');
  await expect.poll(state).toEqual(['suspended']);
...
test('shows bridge and railway scenery while retaining the active route', async ({ page }) => {
  ...
  await (await control(page, page.locator('#crossing-view'))).click();
