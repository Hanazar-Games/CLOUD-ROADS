import { expect, test } from '@playwright/test';
import { control } from './settings';

test('validates climb ranges, applies the eighteen-bend preset and restores natural terrain', async ({ page }) => {
  await page.goto('/?seed=CLOUD-ROAD-001');
  const min = page.locator('#climb-min'), max = page.locator('#climb-max'), mode = page.locator('#elevation-mode');
  const apply = page.getByRole('button', { includeHidden: true, name: '应用并返回起点' });
  await expect(min).toBeDisabled();
  await (await control(page, page.locator('#terrain-kind'))).selectOption('karst');
  await (await control(page, page.locator('#eighteen-bends'))).click();
  await expect(min).toBeEnabled();
  await expect(page.locator('#route-style')).toHaveValue('5');
  await expect(page.locator('#max-grade')).toHaveValue('25');
  await (await control(page, min)).fill('700'); await (await control(page, max)).fill('300');
  await (await control(page, apply)).click();
  expect(await max.evaluate((input: HTMLInputElement) => input.validity.valid)).toBe(false);
  await (await control(page, mode)).selectOption('natural'); await (await control(page, apply)).click();
  await expect(page.locator('#settings-status')).toContainText('喀斯特峰林');
  await expect(min).toBeDisabled();
  await (await control(page, page.locator('#eighteen-bends'))).click();
  await (await control(page, min)).fill('350'); await (await control(page, max)).fill('750'); await (await control(page, apply)).click();
  await expect(page.locator('#settings-status')).toContainText('目标爬升 350–750 米');
  await expect(page.locator('[data-metric="Climb range"]')).toHaveText('350–750 m');
  await expect(page.locator('[data-metric="Road ready"]')).toHaveText('yes', { timeout: 20000 });
  await expect(page.locator('[data-metric="Arch bays"]')).toHaveText('0');
  await expect(page.locator('[data-metric="Arch ribs"]')).toHaveText('0');
  await expect.poll(async () => Number(await page.locator('[data-metric="Bridge piers"]').textContent())).toBeGreaterThan(0);
  await (await control(page, page.locator('#seed'))).fill('CLIMB-OPTIONS');
  await (await control(page, page.getByRole('button', { includeHidden: true, name: '加载种子' }))).click();
  await expect(min).toHaveValue('350'); await expect(max).toHaveValue('750');
  await expect(mode).toHaveValue('cycles');
  await (await control(page, page.locator('#max-grade'))).fill('0'); await (await control(page, apply)).click();
  await expect(page.locator('[data-metric="Maximum grade"]')).toHaveText('0%');
  await expect(page.locator('[data-metric="Road grade"]')).toHaveText('0.00%');
  await expect(page.locator('#error')).toBeHidden();
});

test('renders all new terrains and finds their service areas', async ({ page }) => {
  test.setTimeout(150000);
  const errors: string[] = [];
  page.on('pageerror', error => errors.push(error.message));
  const metric = (name: string) => page.locator(`[data-metric="${name}"]`);
  await page.goto('/?seed=CLOUD-ROAD-001');
  await expect(page.locator('#terrain-kind option')).toHaveCount(10);
  for (const [terrain, name] of [['meadow', '草甸丘陵'], ['badlands', '红岩荒原'], ['karst', '喀斯特峰林'], ['volcanic', '火山高地'], ['tundra', '冰蚀苔原'], ['autumn', '阔叶丘陵']]) {
    await (await control(page, page.locator('#terrain-kind'))).selectOption(terrain);
    await (await control(page, page.getByRole('button', { includeHidden: true, name: '应用并返回起点' }))).click();
    await expect(metric('Landscape')).toHaveText(name);
    await expect(metric('Road ready')).toHaveText('yes', { timeout: 20000 });
    await expect(metric('Active chunks')).toHaveText('289', { timeout: 20000 });
    await expect(metric('Pending / queued')).toHaveText('0 / 0', { timeout: 20000 });
    await (await control(page, page.locator('#service-view'))).click();
    await expect(metric('Service areas')).toHaveText('1', { timeout: 30000 });
    await expect(metric('Pending / queued')).toHaveText('0 / 0', { timeout: 20000 });
    await expect(page.locator('#service-view')).toHaveText('下一服务区');
    await expect(page.locator('#error')).toBeHidden();
  }
  expect(errors).toEqual([]);
});
