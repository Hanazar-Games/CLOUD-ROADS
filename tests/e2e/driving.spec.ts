...
test('switches views and tuning, isolates settings and announcements, and resumes with fresh keys', async ({ page }) => {
  ...
  await (await control(page, page.locator('#driving-fov'))).fill('85');
  await (await control(page, page.locator('#camera-distance'))).fill('10');
  await (await control(page, page.locator('#camera-height'))).fill('0.25');
  await (await control(page, page.locator('#suspension'))).selectOption('1');
  ...
  const reading = await metric(page, 'Vehicle position').textContent();
  await page.keyboard.press('KeyR'); await page.keyboard.press('KeyC');
  await page.waitForTimeout(350);
  await expect(metric(page, 'Vehicle position')).toHaveText(reading!);
  ...
  await page.keyboard.up('KeyW');
  await (await control(page, page.locator('#seed'))).fill('DRIVING-RELOAD');
...
test('keeps driving controls and instruments reachable in small windows', async ({ page }) => {
  ...
  for (const size of [{ width: 390, height: 844 }, { width: 1024, height: 450 }]) {
    ...
    await toggleSettings(page);
    await (await control(page, page.locator('#suspension'))).scrollIntoViewIfNeeded();
...
test('boards on a bridge and a distant highway service area after origin rebasing', async ({ page }) => {
  ...
  await (await control(page, page.locator('#bridge-view'))).click(); await start(page);
